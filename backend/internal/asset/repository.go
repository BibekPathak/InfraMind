...
func (r *Repository) Create(ctx context.Context, a *Asset) error {
	...
	orgID := tenant.EffectiveOrgID(ctx)

	_, err := r.pool.Exec(ctx,
		`INSERT INTO assets (id, name, type, autonomy_mode, location, metadata, organization_id, created_at, updated_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $8)`,
		a.ID, a.Name, a.Type, a.AutonomyMode, loc, meta, orgID, time.Now().UTC(),
	)
	if err != nil {
...
func (r *Repository) List(ctx context.Context) ([]Asset, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT id, name, type, autonomy_mode, location, metadata, created_at, updated_at
		 FROM assets WHERE organization_id = $1 AND deleted_at IS NULL ORDER BY created_at DESC`,
		tenant.EffectiveOrgID(ctx))
	if err != nil {
...
func (r *Repository) UpdateAutonomyMode(ctx context.Context, id, mode string) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE assets SET autonomy_mode = $2, updated_at = NOW() WHERE id = $1 AND organization_id = $3 AND deleted_at IS NULL`,
		id, mode, tenant.EffectiveOrgID(ctx))
	if err != nil {
...
func (r *Repository) GetAutonomyMode(ctx context.Context, id string) (string, error) {
	var mode string
	err := r.pool.QueryRow(ctx,
		`SELECT autonomy_mode FROM assets WHERE id = $1 AND organization_id = $2 AND deleted_at IS NULL`, id, tenant.EffectiveOrgID(ctx),
	).Scan(&mode)
	if err != nil {
...
func (r *Repository) Delete(ctx context.Context, id string) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE assets SET deleted_at = $2, updated_at = $2 WHERE id = $1 AND organization_id = $3 AND deleted_at IS NULL`,
		id, time.Now().UTC(), tenant.EffectiveOrgID(ctx),
	)
	if err != nil {
