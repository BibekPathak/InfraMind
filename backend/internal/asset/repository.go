...
func (r *Repository) Create(ctx context.Context, a *Asset) error {
	...
	_, err := r.pool.Exec(ctx,
		`INSERT INTO assets (id, name, type, location, metadata, created_at, updated_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $6)`,
		a.ID, a.Name, a.Type, loc, meta, time.Now().UTC(),
	)
	if err != nil {
...
func (r *Repository) GetByID(ctx context.Context, id string) (*Asset, error) {
	...
	err := r.pool.QueryRow(ctx,
		`SELECT id, name, type, location, metadata, created_at, updated_at, deleted_at
		 FROM assets WHERE id = $1 AND deleted_at IS NULL`, id,
	).Scan(&a.ID, &a.Name, &a.Type, &loc, &meta, &a.CreatedAt, &a.UpdatedAt, &deletedAt)
	if err != nil {
...
func (r *Repository) List(ctx context.Context) ([]Asset, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT id, name, type, location, metadata, created_at, updated_at
	...
	for rows.Next() {
		var a Asset
		var loc, meta []byte
		if err := rows.Scan(&a.ID, &a.Name, &a.Type, &loc, &meta, &a.CreatedAt, &a.UpdatedAt); err != nil {
